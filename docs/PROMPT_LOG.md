...
> 의도/반영: README·PRD·SRD·가이드 모달 기준으로 기능 체크리스트를 만들고, **수동 확인 + 코드 대조 + 저장소 밖 일회성 Playwright**로 검증(repo에 테스트 스위트 추가 없음). 대부분 정상. **버그**: 선택 모드 Enter → 아래 셀 이동(별도 항목에서 수정). **UX·문서**: 가이드 모달 키보드 누수·PRD/SRD Export 제목·모바일 문구 불일치 → 후속 반영(`isGridKeyboardTarget`, PRD/SRD/TRD 동기화).
...
```
전체 코드에서 안 쓰는 부분이랑 중복된 로직 정리해 줘. 바꾸기 애매한 건 먼저 물어봐.
```

> 의도/반영: 죽은 코드·중복 줄이고, 선택 UI는 `updateCellsUI` 한 곳에서만 갱신. 행/열 삭제·undo·제목 편집 쪽 자잘한 버그도 같이 손봄. 큰 선택지 네 가지는 다음 프롬프트에서 확정.
...
```
아까 말한 1번부터 4번까지 순서대로 해 줘. 문서도 코드에 맞게 고쳐 줘.
```

> 의도/반영: 마지막 1행·1열일 때는 삭제 메뉴 비활성화, 그리드는 크기 안 바뀌면 값만 맞추고 크기 바뀔 때만 통째로 다시 그림, 행/열 span 계산 통합, TRD·SRD·IME 문서·README 표현 정리.
...
```
리팩터링한 뒤부터 셀이랑 행·열 드래그 선택이 안 되고 방향키로도 안 움직여. 예전에 넣어 둔 기능인데 깨진 것 같아. 원인 찾아서 고쳐 줘.
```
